...
public  void StopTime()
    ...
    public void Start(DateTime startTime, DateTime endTime)
    {
        ...
        if(tof == null) tof = new TimeObjFactory(deferred);  // false by default
...
OneToDt(DateTime dt) {
    ...
    public static IEnumerable<TimeObj>  //DateTime.Now //fun(x.Key)
ToDt(   Predicate<DateTime> fun   )
    {

        var retVal =
                 from x in timeObjDictionary
                 where fun( x.Key )       //x.Key ==  DateTime.Now.Neuter()  // DateTime.Now.Day
                 select x.Value; 
